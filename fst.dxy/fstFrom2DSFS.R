...
#outprefix="RRivTog00186-xxxxEth00164"
##read in full 2dsfs:
#full<-scan("data/transposed.RRivTog00186-xxxxEth00164_unfolded.2dsfs", skip=1)
full<-scan(insfs, skip=1)
fullM<-matrix(full, ncol=cols, nrow=rows, byrow=T)
...
out <- data.frame(name=outprefix, fstSS=getFst(fullM))
cat(out[[1]], out[[2]],fill=T, sep="\t")
